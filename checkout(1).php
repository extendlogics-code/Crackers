...
<span style="font-size:10px;"><strong>* Shipping charges include delivery for up to 1 kg. Additional courier fees apply for any weight beyond this limit.</strong></span>
    ...
    <script>
      ...
      confirmBtn.addEventListener('click', async ()=>{
        f.txn_id.value = txnInput.value.trim();
        if (!validateField('txn_id')) { txnInput.focus(); return; }
        ...
        try {
          const payload = buildPayload({ transaction_id: f.txn_id.value.trim() });
      ...
      cancelQrBtn.addEventListener('click', closeQr);
